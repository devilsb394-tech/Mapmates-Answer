...
5. WEBSITE INTENT DETECTION: If the user is searching for a specific website, brand, or service (such as "YouTube", "Google", "Facebook", "Twitter", etc.) or wants to find a site to watch/visit:
   - YOU MUST place the official, clean link to that website (e.g. 'https://www.youtube.com', 'https://www.facebook.com', 'https://www.google.com') at the very top of the 🌐 LINKS section.
   - Strictly filter out general question-answer websites, directories, or irrelevant forum links (like Wikipedia, Quora, StackOverflow) that clutter the results when they just want the direct website.
6. QUESTION EXPERT CRITIQUE & RAG RANKING: If the user asks a question (like "what is ...", "how does ...", "is it possible to ...", or any other analytical question):
   - You MUST read the provided Contextual Data (Web Results with their titles, excerpts, and URLs) very carefully.
   - Evaluate who among the sources gave the absolute BEST, most accurate, and most valuable answer to the user's question ("web ko parhe ke kis ne Sahi answer diya hai khud se soche").
   - Rank those best evaluated answers AT THE VERY TOP of the 🌐 LINKS section.
   - Next to each ranked link, write an elegant, brief 1-sentence explanation of why it was evaluated as the top source for this answer (e.g. "- [Channel/Site Name](URL) - **Verified Answer:** This source provides the most comprehensive and verified explanation regarding...").
...
app.post("/api/search", async (req, res) => {
  try {
    ...
    // 1. Pro Search: Multi-step Reasoning & RAG (Data Indexing Speed 1, 2, 4)
    let searchContextStr = "No external search context fetched.";
    let searchLinks: string[] = [];
    try {
      const tavilyApiKey = process.env.TAVILY_API_KEY || "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";

      // Step 1: Query Auto-Decomposition for Pro Search
      const decomposePrompt = `Analyze the query: "${query}". Break it down into 3-4 distinct search queries to gather comprehensive data, similar to Perplexity Pro Search. Return only a JSON array of strings. Example: ["query 1", "query 2"]`;
      const decomposeRes = await callAIWithFallback(decomposePrompt, { maxOutputTokens: 150 });
      let subQueries: string[] = [query];
      
      try {
         const parsed = JSON.parse(decomposeRes.replace(/^```(json)?\n?/, '').replace(/\n?```$/, '').trim());
         if (Array.isArray(parsed) && parsed.length > 0) {
            // Include original query, limit to 4 parallel processing streams (Speed 4)
            subQueries = Array.from(new Set([query, ...parsed])).slice(0, 4);
         }
      } catch (e) {
         // Fallback to simple query if parse fails
      }

      console.log(`[Pro Search Indexing] Multi-step reasoning generated ${subQueries.length} streams...`);

      // Step 2: Parallel Fetching (Data Indexing Speed 1->2->4 wale)
      const searchPromises = subQueries.map(async (sq, idx) => {
         const res = await fetch("https://api.tavily.com/search", {
           ...
           body: JSON.stringify({
             ...
             include_answer: idx === 0, // Only need direct answer for the main query
             max_results: 3
           })
         });
         if (res.ok) {
      ...
      console.log("[Pro Search Indexing] Data aggregated at Speed 4x. Compiling context...");

      let combinedResults: any[] = [];
      let mainAnswer = "";

      allResults.forEach((tavilyData, idx) => {
         if (tavilyData) {
            if (tavilyData.answer && idx === 0) mainAnswer = tavilyData.answer;
            combinedResults = combinedResults.concat(tavilyData.results || []);
         }
      });
      ...
      combinedResults.forEach(r => {
         if (r && r.url && !uniqueResultsMap.has(r.url)) {
            uniqueResultsMap.set(r.url, r);
         }
      });
      const finalResults = Array.from(uniqueResultsMap.values()).slice(0, 8); // Top 8 deep results

      searchLinks = finalResults.map((r: any) => r.url);
      
      searchContextStr = `[PRO SEARCH RAG CONTEXT] (Compiled from ${subQueries.length} reasoning paths):\n\n`;
      finalResults.forEach((r: any, idx: number) => {
        searchContextStr += `[Source ${idx+1}] Title: ${r.title}\nContent: ${r.content}\nURL: ${r.url}\n\n`;
      });
      if (mainAnswer) {
        searchContextStr += `\nMulti-step Synthesized Direct Answer: ${mainAnswer}\n`;
      }

    } catch (searchErr) {
      console.error("Tavily Pro Search error:", searchErr);
      // Fallback: Proceed without external context if search fails
    }

    // AI Processing to generate response
    const systemInstruction = `You are the core intelligence of "MapMates Answer". 
    Process the user's query: "${query}".

    Here is the real-time web context you MUST base your response on:
    ${searchContextStr}

    Provide a structured response containing:
    1. AI Summary: A deep analytical summary incorporating the real-time search context.
    2. Voice Link: A URL to the synthesized voice response (you may generate a placeholder if needed).
    3. Location Data: If relevant, extract location details.
    4. AI Insight: Strategic deep analysis of the topic.
    5. Search Context: List of relevant sources/links extracted from the provided exact mapping.
    ...
    Format:
    {
      ...
      "locationData": "...",
      "aiInsight": "...",
      "searchContext": ${JSON.stringify(searchLinks.length > 0 ? searchLinks : ["link1", "link2"])}
    }
    `;

    const aiRes = await callAIWithFallback(query, {
      systemInstruction: systemInstruction,
      maxOutputTokens: 2000,
    });
    ...
    // Ensure we append real URLs if AI missed tracking them exactly
    if (searchLinks.length > 0) {
      data.searchContext = Array.from(new Set([...(data.searchContext || []), ...searchLinks])).slice(0, 5);
    }
...
app.get("/api/search-ddg", async (req, res) => {
  try {
    ...
    const response = await fetch(ddgUrl, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36' } });
    const html = await response.text();
    const $ = cheerio.load(html);
    $('.result').each((i, el) => {
      ...
      if (title && cleanUrl) finalResults.push({ title, url: cleanUrl, snippet, source: 'duckduckgo' });
    });
  ...
  try {
    ...
    try {
      if (offset === "0") {
         ...
         const parsed = JSON.parse(decomposeRes.replace(/^```(json)?\n?/, '').replace(/\n?```$/, '').trim());
         if (Array.isArray(parsed) && parsed.length > 0) {
            subQueries = Array.from(new Set([query, ...parsed])).slice(0, 4); // Speed 4x parallel indexing
         }
      }
    } catch (e) {}
    ...
    // Core RAG Parallel Indexing (Speed 1, 2, 4 wale)
    const searchPromises = subQueries.map(async (sq, idx) => {
      try {
         const res = await fetch("https://api.tavily.com/search", {
           method: "POST",
    ...
    const tavilyArrays = await Promise.all(searchPromises);
    tavilyArrays.forEach(arr => { combinedResults = combinedResults.concat(arr); });

    let finalResults: any[] = [];
    if (combinedResults.length > 0) {
       const uniqueMap = new Map();
       combinedResults.forEach(r => {
          if (r.url && !uniqueMap.has(r.url)) uniqueMap.set(r.url, r);
       });
    ...
    if (query.toLowerCase().includes('youtube') || query.toLowerCase().includes('video')) {
       try {
           const dmRes = await fetch(`https://api.dailymotion.com/videos?fields=id,title,thumbnail_720_url,duration,owner.screenname,url&search=${encodeURIComponent(query)}&limit=3`);
           if (dmRes.ok) {
              const dData = await dmRes.json();
              if (dData.list && dData.list.length > 0) {
                 dData.list.forEach((v: any) => {
                     finalResults.unshift({
                        title: `Video Match: ${v.title}`,
                        url: v.url,
                        snippet: `High-quality integrated video result from ${v["owner.screenname"]}`,
                        source: 'dailymotion'
                     });
                 });
              }
...
// MapMates Motion API (Video Feed)
app.post("/api/motion", async (req, res) => {
  try {
    ...
    let searchStr = query;
    if (page === 1) {
      const systemInstruction = `You are the MapMates Motion analyzer.
      The user wants short videos/reels. 
      Analyze their query: "${query}".
      Fix any spelling mistakes, understand their core intent, and return ONLY a clean search phrase (1-5 words) that would yield the best results on Pexels video search.
      DO NOT return any other text, JSON, or conversational responses. ONLY the search query.`;
    ...
    // Array of fetching promises
    const pexelsPromise = (async () => {
       const res = await fetch(`https://api.pexels.com/videos/search?query=${encodeURIComponent(searchStr)}&orientation=portrait&per_page=15&page=${page}`, {
         headers: { Authorization: "UmKpgy8iaAfNcdefHOS2XbVIkm29hzWIeh9Qppql7JUnAHwWbWCQeY8n" }
       });
       if (!res.ok) return [];
       const data = await res.json();
       return (data.videos || []).map((v: any) => ({
         ...
         title: v.url?.split('/').pop()?.replace(/-/g, ' ') || 'Motion Video'
       }));
    })().catch(() => []);

    const dmPromise = (async () => {
       // Search Dailymotion for short vertical-ish videos
       const res = await fetch(`https://api.dailymotion.com/videos?fields=id,title,thumbnail_720_url,duration,owner.screenname,url&search=${encodeURIComponent(searchStr)}&limit=10&page=${page}&shorter_than=65`);
       if (!res.ok) return [];
       const data = await res.json();
       return (data.list || []).map((v: any) => ({
         id: `dm_${v.id}`,
         title: v.title,
         image: v.thumbnail_720_url,
         iframeSrc: `https://www.dailymotion.com/embed/video/${v.id}?autoplay=0&muted=1&ui-logo=0&sharing-enable=0`,
         platform: 'dailymotion',
         creatorName: v['owner.screenname'] || 'Dailymotion'
       }));
    })().catch(() => []);

    const biliPromise = (async () => {
       // Search Bilibili for videos
       const res = await fetch(`https://api.bilibili.com/x/web-interface/search/all/v2?keyword=${encodeURIComponent(searchStr + ' shorts')}&page=${page}`, { 
         headers: { 'User-Agent': 'Mozilla/5.0' }
       });
       if (!res.ok) return [];
       const data = await res.json();
       const vData = data.data?.result?.find((r: any) => r.result_type === 'video');
       return (vData?.data || []).slice(0, 10).map((v: any) => ({
         id: `bili_${v.bvid}`,
         title: v.title.replace(/<[^>]*>/g, ''),
         image: v.pic?.startsWith('//') ? `https:${v.pic}` : v.pic,
         iframeSrc: `https://player.bilibili.com/player.html?bvid=${v.bvid}&autoplay=0&high_quality=1`,
         platform: 'bilibili',
         creatorName: v.author || 'Bilibili'
       }));
    })().catch(() => []);
    ...
    const ptPromise = (async () => {
       ...
       const res = await fetch(`https://peertube.cpy.re/api/v1/videos?search=${encodeURIComponent(searchStr)}&count=10&start=${start}`);
       if (!res.ok) return [];
       const data = await res.json();
       return (data.data || []).map((v: any) => ({
         id: `pt_${v.uuid}`,
         title: v.name,
         image: `https://peertube.cpy.re${v.thumbnailPath}`,
         iframeSrc: `https://peertube.cpy.re/videos/embed/${v.uuid}?autoplay=0`,
         platform: 'peertube',
         creatorName: v.account?.displayName || 'PeerTube'
       }));
    })().catch(() => []);
    
    // Add Archive.org to the mix
    const archivePromise = (async () => {
       const res = await fetch(`https://archive.org/advancedsearch.php?q=title:(${encodeURIComponent(searchStr)})+AND+mediatype:(movies)&output=json&rows=5&page=${page}`);
       if (!res.ok) return [];
       const data = await res.json();
       return (data.response?.docs || []).map((v: any) => ({
         id: `ar_${v.identifier}`,
         title: v.title,
         image: `https://archive.org/services/img/${v.identifier}`,
         iframeSrc: `https://archive.org/embed/${v.identifier}?autoplay=0`,
         platform: 'archive',
         creatorName: v.creator || 'Archive'
       }));
    })().catch(() => []);

    const [pexelsVideos, dmVideos, biliVideos, ptVideos, archiveVideos] = await Promise.all([
      pexelsPromise, dmPromise, biliPromise, ptPromise, archivePromise
    ]);

    // Merge and shuffle
    let allVideos = [...pexelsVideos, ...dmVideos, ...biliVideos, ...ptVideos, ...archiveVideos];
    allVideos = allVideos.sort(() => Math.random() - 0.5);
    
    // Add pseudo-AI rating and description for each video
    const enhancedVideos = allVideos.map((video: any) => {
      const benefits = [
        "Provides visual clarity on the topic.",
        "Enhances understanding through dynamic motion.",
        "Offers a relaxing visual experience.",
        "Inspires creativity and new ideas.",
        "Delivers quick, digestible information."
      ];
      const randomBenefit = benefits[Math.floor(Math.random() * benefits.length)];
      return {
        ...video,
        aiDescription: `Observation: ${video.creatorName}'s visual. 
Benefit: ${randomBenefit}`
    ...
    res.json({ 
      ...
      queryUsed: searchStr,
      hasMore: enhancedVideos.length > 0
    });

  } catch (err) {
    console.error("Motion API Error:", err);
    res.status(500).json({ error: "Video feed failed" });
  }
});
